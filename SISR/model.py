...
####### The single GPU model
        if is_train:
            self.test_Inputs1, self.test_Labels1, self.test_Inputs2, self.test_Labels2, self.test_Inputs3, self.test_Labels3, self.test_Inputs4, self.test_Labels4 = self.prepare_data('Set14')
        else:
            self.test_Inputs1, self.test_Labels1 = self.prepare_data('BSD')
...
############################### MODEL ###############################################################################
    ...
    def get_loss(self, input_ph, label_ph, scope = 1):
        y_hat = self.build_graph(input_ph, label_ph)
...
################################################### function #######################################
    def recon(self, features, _recon, _noise, SR, name):
        with tf.variable_scope(name):
            ...
            down = bicubic_interp_2d(_recon, [int(_recon_h1 / SR), int(_recon_h2 / SR)])
...
####################################### TRAIN ###########################################
    ...
    ############################################## load Train data ############################################
        ...
        train_Inputs1, train_Labels1, train_Inputs2, train_Labels2 , train_Inputs3, train_Labels3, train_Inputs4, train_Labels4 = self.prepare_data('train')#train  Set14
  ...
  ############################################## Prepare Train data ############################################
            ...
            ########################## The validate running ####################33
                ...
                if counter % (save_step) == 0:  # save_step
                    ...
                    # if avg_LOSS_validate < tmp:
                    #     self.save(config.checkpoint_dir, counter, config.model_dir)
                    #     tmp = avg_LOSS_validate
                    #     tmp_psnr = avg_PSNR_validate
            #             print("//------------------------------[Y save Y][avg_PSNR]all_validate_PSNR: %2.4f,  [LOSS] all_validate_loss: %.8f------------------------------//" % (tmp_psnr, tmp))
            # print("//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~[Epoch %d][avg_trn_loss: %.8f" % (epoch + 1, LOSS_trn/batch_idxs))
    ...
    def prepare_data(self, key='BDS'):
        ...
        """
        if key == 'BSD':
            input_path = './SR_2/' + key + '_input.mat'
            label_path = './SR_2/' + key + '_label.mat'
            ...
            input_path = './fli_BSD91_32_d2_47w/' + key + '_input.mat'
            label_path = './fli_BSD91_32_d2_47w/' + key + '_label.mat'
    ...
    def test(self, config, prin):
        ...
        # PSNR_avg = PSNR
        # scipy.io.savemat('./result/SR3_BSD.mat', {'Im': test_out_lst})
