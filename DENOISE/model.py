...
####### The single GPU model
        if is_train:
            self.test_Inputs1, self.test_Labels1, self.test_Inputs2, self.test_Labels2, self.test_Inputs3, self.test_Labels3, self.test_Inputs4, self.test_Labels4 = self.prepare_data('Set12')
        else:
            self.test_Inputs1, self.test_Labels1 = self.prepare_data_test('BSD68')# BSD  BSD68 Set12_256 Set12_512
...
####################################### TRAIN ###########################################
    ...
    ############################################## load Train data ############################################
        ...
        train_Inputs1, train_Labels1, train_Inputs2, train_Labels2 , train_Inputs3, train_Labels3, train_Inputs4, train_Labels4 = self.prepare_data('train')#train  Set12
    ...
    ############################################# load validate data ############################################
        ...
        self.val_Inputs1, self.val_Labels1, self.val_Inputs2, self.val_Labels2 , self.val_Inputs3, self.val_Labels3, self.val_Inputs4, self.val_Labels4 = self.prepare_data('validate')#validate
        ...
        for epoch in xrange(config.epoch):
            ...
            save_step = np.array(batch_idxs// 10).astype(int)

            # if epoch == 14:
            #     learning_rate = config.learning_rate * 0.6
            # elif epoch == 24:
            #     learning_rate = config.learning_rate * 0.6**2
            # elif epoch == 31:
            #     learning_rate = config.learning_rate * 0.6**3
            # elif epoch == 36:
            #     learning_rate = config.learning_rate * 0.6 ** 3
  ...
  ############################################## Prepare Train data ############################################
    ...
    def recon(self, features, _recon,_noise, name):
        with tf.variable_scope(name):
            ...
            recon = tf.multiply((1-delta-eta), features) + tf.multiply(eta, _recon) + tf.multiply(delta, _noise)
        return recon

    def recon_out(self, conv_out, _recon):
        with tf.variable_scope("recon_out"):
            delta = tf.get_variable(name='delta', initializer=tf.constant(0.5))
            eta   = tf.get_variable(name='eta', initializer=tf.constant(0.5))
            recon = tf.multiply(eta, _recon) + tf.multiply(delta, conv_out)
        return recon


    def Block(self,  x, channels=64, kernel_size=[3, 3], scale =1):
        with tf.variable_scope("Block"):
            tmp = slim.conv2d(x, channels, kernel_size, activation_fn=tf.nn.relu, scope = 'conv1')
            tmp = slim.conv2d(tmp, channels, [5, 5], activation_fn=None, scope = 'conv2')
            tmp *= scale
        return x + tmp
    def prepare_data_test(self, key):
        ...
        """
        input_path = './test_15/' + key + '_input.mat'
        label_path = './test_15/' + key + '_label.mat'
    ...
    def prepare_data(self, key='test'):
        """prepare data
        ...
        """
        if key=='test':
            input_path = './data/' + key + '_input.mat'
            label_path = './data/' + key + '_label.mat'
            ...
            input_path = './Noise15_50_60w/' + key + '_input.mat'
            label_path = './Noise15_50_60w/' + key + '_label.mat'
    ...
    def test(self, config, prin):
        ...
        for idx in xrange(0, self.test_Inputs1.shape[0]):
             ...
             PSNR_mat.append(PSNR)
             # save_images( de_img, [1,1], './sample/imag_%d.png'%idx )

        PSNR_avg = PSNR_all/num
        PSNR_mat.append(PSNR_avg)
        # PSNR_avg = PSNR

        # scipy.io.savemat('./data/Set12_512_denoise.mat', {'Im': test_out_lst})
        print("[INFO] [test] test finished, PSNR is %2.4f, cost time is: %.4f" % (PSNR_avg, (time.clock() - start_time)))
        # scipy.io.savemat('./sample/PSNR_mat.mat', {'PSNR_mat': PSNR_mat})

        # Set12_256_denoise   Set12_512_denoise  BSD68_denoise